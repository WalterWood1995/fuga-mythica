...
**v0.49(2026-09-13)德语第 7 批** `root_stories_de7.js`(17 篇长文:sinken/meinen/trennen/üben/hindern/tasten/Trost/zögern/Wind/Regen/Eis/Amt/Punkt/Kreis/Ort/Sprache/Dienst/Ordnung + angst/flug/fluss/ordn/dien 复用同族故事)。**德语 326 篇,覆盖 4160/6607(63%)。**

**v0.50(2026-09-13)德语第 8 批** `root_stories_de8.js`(19 篇:schön/schnell/hoffen/wachsen/schwimmen/Vogel/Pferd/Ding/Wald/Stern/Morgen/Abend/Milch/Schiff/schwach/König/zer-/miss-/-ig + pflicht 复用 pflegen)。**德语 346 篇,覆盖 4280/6607(65%)。** 剩余约 400 个德语本族词干(spring/werf/lieg/wähl/holz/hass/weich/leer/atm/schweig/prüf/rett/wag/schmeck/stift/tauch/quäl/gesamt/sonder/platz2/schnee/seit/brief/antwort/wein/tanz/baum/fisch/nam/still/wieder/zusammen/stund/heim/fenster/stück/auf/lich/zwing/sterb/eisen/kunst/sommer/winter/link/zweck/spaß/back/näh/erlaub/sammel/stör/schweb/speicher/spreng/fleisch/salz/ware/rand…)。
...
**v0.54(2026-09-14)德语第 12 批** `root_stories_de12.js`(11 篇:Stück/auf-/-lich(含 Leiche、gleich 全族)/zwingen/sterben/Eisen/Kunst/Sommer/Winter/links/Zweck + fenster 复用共用 fenestr)。**德语 395 篇,覆盖 4548/6607(69%)。**

**v0.55(2026-09-14)德语第 13 批** `root_stories_de13.js`(10 篇:Spaß/backen/nähen(含 Nadel、Naht、spinnen、weben 全族)/erlauben(含 Urlaub、Glaube)/stören(含 Sturm、stürzen)/schweben/Speicher/sprengen/Fleisch/Salz)。**德语 405 篇,覆盖 4598/6607(70%)。**

**v0.56(2026-09-14)德语第 14 批** `root_stories_de14.js`(8 篇长文:Ware(含 wahren、Währung、Gewähr 全族)/Rand/kurz/schwer/leicht(含 Lunge)/freuen(含 Schadenfreude、Vorfreude)/Kampf(拉丁 campus!)/Hund)。**德语 413 篇,覆盖 4632/6607(70%)。**

**v0.57(2026-09-14)德语第 15 批** `root_stories_de15.js`(7 篇长文:laut(含 Laut 语音学术语、Schall、Leumund)/dunkel/ober-(含 Obrigkeit)/Woche(含七日名的由来)/können(含 kennen、Kenntnis、Bekenntnis、Kunde 全族)/nah(含 Nachbar、Nächstenliebe)/vor- + platz 复用)。**德语 421 篇,覆盖 4664/6607(71%)。**
...
**v0.67(2026-09-22)地图词根补全** 用户要求把地图冒险里缺词根的约 40% 补上,强调 app 的核心是用词根词缀给单词归类、用词源帮助记忆,各语言内容可以不同。做法:①逐词按真实词源手工指定词根(不用自动匹配——自动匹配会把 la vipère 配到 père),法/意/西按共同拉丁祖词归组,一个故事三语共用;②`root_stories11.js` 新增 177 个共用词根故事(含 petit/blanc/bleu/piccolo/niño/perro/pequeño 这些以前因来源不明而留空的词,现在如实写\"来源有争议\"),`root_stories_de23.js` 新增 121 个德语本族词根故事;③`tgroots.js` 把每个词挂到正确词根(词根包里没有的词就补进去),并支持两种覆盖:`!` 强制改挂(修正了 la tour 被挂在\"转\"、il ritorno 挂在\"绕\"、Trauer 挂在 treu 等错误),`=:` 只指定高亮字母(la fille 高亮 fill、le vœu 高亮 vœu);德语的拉丁借词通过 deMap 读共用故事。审计结果:德法意西四种语言 400 个地图词全部有真实词根、有来历故事、词根字母能在词里高亮。已知缺口:约 170 个/语言的新词根目前只有这一个词,答案卡的\"同根词族\"表为空(故事正文里已列出同族词)。

**v0.68(2026-09-22)同根词族补全** `tgfam.js` 为地图冒险用到的 688 个词根(德 166、法 174、意 177、西 171)各补 2–4 个真正同根的词(带中英释义),答案卡的\"🔗 同根词族\"表从约 170 个/语言为空,变成德 396、法 391、意 395、西 395 个词有词族(其余是 diadème 这类本身很孤立的词)。写入前做了一遍事实核查,删掉 57 个形近但不同根的候选(如 Wiesel≠Wiese、farouche≠ferus、Murmeltier≠murmeln、le donjon≠turris、el zurdo≠izquierdo)。词族表的词根高亮命中率:德 94%、意 87%、法 82%、西 80%。

**v0.69(2026-09-24)词源页结构化 `etymview.js`** 用户反馈词源解释页太乱,要求用结构化、可分层收起的方式呈现。做法:①把一整段故事解析成结构——印欧词根(1032/1575 个故事能自动识别)、同源词(按语言抽成词条,中位 4 个)、句子;②顶部一个小思维脑图:词根节点 + 若干分支(🌍 最早的词根 / 🧬 同源词 / 🧩 同根词族 / 🏛️ 拉丁源头 / 📜 词源故事),点分支展开对应层;③每层是 `<details>`,默认只展开第一层;④故事按句分段显示,**括号感知的断句**(故事里大量感叹号在括号内,普通断句会把句子切碎);⑤德语那种上百个复合词的长枚举自动转成词条卡片网格(如 klettern 一句 85 个复合词),并保留\"原文\"折叠。三处入口统一改用它:地图答案卡(含拉丁语的衍生词表与\"今天还活着\"也收进同一结构)、词根课堂、速认的词根故事列表。
